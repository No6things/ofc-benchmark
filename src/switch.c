...
void ofp13SwitchLearnDstmac(struct fakeswitch *fs){
    ...
    char buf [512];
    int len = sizeof( struct ofp131_packet_in )+ 32 + sizeof(gratuitous_arp_reply);
    ...
    OFP_OXM_SHDR_FIELD(oxm_ptr,0x15);  //OFPXMT_OFB_ARP_OP = 21, ARP
		...
		debugMsg(fs, "Size of pkt_in before gratuitous_arp_reply: %d ", pkt_in);
...
static int makeStatsDescReply(struct ofp_stats_request * req,
    ...
    static struct ofp_desc_stats ofcbench_desc = {
		...
		strcpy(ofcbench_desc.sw_desc, VERSION);
    struct ofp_stats_reply * reply;
    int len = sizeof(struct ofp_stats_reply) +
...
static int ofp13MakePacketIn(int switch_id, int xid, int buffer_id, char * buf, int buflen, int mac_address){
    ...
    const char fake[] = {
                0x97,0x0a,0x00,0x52,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x01,0x00,0x40,0x00,0x47,
								...
								0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x02,0x08,0x00,0x45,0x00,0x00,0x32,
								0x00,0x00,0x00,0x00,0x40,0xff,0xf7,0x2c,0xcc,0xa8,0x00,0x40,0xc0,0xa8,0x01,0x28,
    ...
    while((count= msgbufCountBuffered(fs->inbuf)) >= sizeof(struct ofp_header ))
    {
        ...
        switch(ofph->type)
        {
            ...
            case OFPT131_MULTIPART_REQUEST:
                ...
                switch (mp_req_type)
                {
                    ...
                    case OFPMP_TABLE_FEATURES:
                        debugMsg(fs, "OF1.3 Got MP table feature req,"
                                " ignored");
                        break;
                ...
                break;
            /*case OFPT131_SET_CONFIG: made by kc-cbench
                debugMsg(fs, "OF1.3 got set config, ignored");
                break;*/
            ...
            case OFPT131_ROLE_REQUEST:
                debugMsg(fs, "OF1.3 got role request, ignored");
                break;
